...
DoubleMapper featureMapper(MeasuredFeatures features1, MeasuredFeatures features2) {
    ...
    final filteredFeatures1 = [for (final f in features1) if (f.feature is Corner) f];
    final filteredFeatures2 = [for (final f in features2) if (f.feature is Corner) f];

    final featureProgressMapping = doMapping(filteredFeatures1, filteredFeatures2);

    _log.fine(featureProgressMapping.joinToString { "${it.first} -> ${it.second}" });
